...
# Конфигурация бота
BOT_TOKEN = "8524664343:AAGtxSCtrcA-abb0ye2pqZjF3o10HQKE5Yc"
ADMIN_ID = 1565932131  # Telegram ID администратора
...
class Database:
    ...
    def _add_missing_columns(self):
        ...
        required_user_columns = ['gender', 'birth_date', 'has_children']
        for col in required_user_columns:
            if col not in user_columns:
                ...
                elif col == 'has_children':
                    self.cursor.execute('ALTER TABLE users ADD COLUMN has_children BOOLEAN DEFAULT 0')
    ...
    def _update_constraints(self):
        try:
            ...
            ''')
            self.cursor.execute('INSERT INTO applications_new SELECT * FROM applications')
            self.cursor.execute('DROP TABLE applications')
            self.cursor.execute('ALTER TABLE applications_new RENAME TO applications')
            self.cursor.execute("COMMIT")
            self.cursor.execute("PRAGMA foreign_keys=ON")
        except Exception as e:
            logging.error(f"Ошибка обновления ограничений: {e}")
            self.cursor.execute("ROLLBACK")

    def add_user(self, user_id, full_name, phone, username=None, gender=None, birth_date=None, has_children=None):
        ...
        if exists:
            self.cursor.execute('''
                UPDATE users SET
                    ...
                    has_children = ?
                WHERE user_id = ?
            ''', (full_name, phone, username, gender, birth_date, 1 if has_children else 0, user_id))
        else:
            self.cursor.execute('''
                INSERT INTO users
                (user_id, full_name, phone, username, gender, birth_date, has_children)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, full_name, phone, username, gender, birth_date, 1 if has_children else 0))
        self.conn.commit()
    ...
    def get_targeted_users(self, gender=None, has_children=None):
    ...
    def get_applications_by_event(self, event_id):
        try:
            query = '''
                SELECT a.id as application_id, u.full_name, u.phone, u.username, u.gender, u.birth_date, u.has_children, a.applied_at
                FROM applications a
    ...
    def export_event_applications_to_excel(self, event_id):
        try:
            ...
            df = pd.DataFrame(applications, columns=[
                ...
                'gender', 'birth_date', 'has_children', 'applied_at'
            ])
            buffer = BytesIO()
            with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
...
class RegistrationStates(StatesGroup):
    ...
    waiting_for_has_children = State()
...
class EventBot:
    ...
    def _register_handlers(self):
        ...
        self.router.callback_query.register(self.process_has_children, F.data.startswith('children_'), StateFilter(RegistrationStates.waiting_for_has_children))
        self.router.message.register(self.process_contact, F.content_type == ContentType.CONTACT, StateFilter(RegistrationStates.waiting_for_phone))
    ...
    async def process_consent(self, callback_query: types.CallbackQuery, state: FSMContext):
        ...
        await callback_query.message.answer("Введите ваше полное имя (ФИО):")
    ...
    async def process_birth_date(self, message: types.Message, state: FSMContext):
        ...
        try:
            ...
            keyboard = InlineKeyboardBuilder()
            keyboard.button(text="Есть дети 👶", callback_data="children_yes")
            keyboard.button(text="Нет детей 👤", callback_data="children_no")
            keyboard.adjust(2)
            await message.answer("Есть ли у вас дети?", reply_markup=keyboard.as_markup())
            await state.set_state(RegistrationStates.waiting_for_has_children)
        except ValueError:
            await message.answer("Неверный формат даты. Пожалуйста, введите дату в формате ДД.ММ.ГГГГ (например, 01.01.1990).")

    async def process_has_children(self, callback_query: types.CallbackQuery, state: FSMContext):
        has_children = callback_query.data == 'children_yes'
        await state.update_data(has_children=has_children)
        await callback_query.answer()
        keyboard = ReplyKeyboardBuilder()
        keyboard.button(text="📱 Отправить контакт", request_contact=True)
        await callback_query.message.answer(
            "Добавьте ваш номер телефона. Вы можете ввести его вручную "
            "или нажать кнопку ниже для отправки контакта.",
    ...
    async def process_contact(self, message: types.Message, state: FSMContext):
        ...
        await self._save_user_data(message, state, phone)
    ...
    async def process_phone_manual(self, message: types.Message, state: FSMContext):
        ...
        await self._save_user_data(message, state, clean_phone)

    async def _save_user_data(self, message, state, phone):
        user_id = message.from_user.id
        ...
        self.db.add_user(user_id, full_name, phone, username, gender, birth_date, has_children)
        ...
        await state.update_data(user_name=user_name, registered=True)
    ...
    async def show_menu(self, message: types.Message):
        ...
        user = self.db.get_user(user_id)
        if not user:
            state = FSMContext(storage=self.storage, chat_id=message.chat.id, user_id=user_id)
            data = await state.get_data()
        ...
        if not events:
            ...
            await message.answer(
                ...
                "Пожалуйста, ожидайте обновлений или используйте меню ниже для навигации.",
    ...
    def _create_target_selection_keyboard(self, context):
        ...
        keyboard.button(text="Женщинам с детьми 👩👶", callback_data=f"target_female_children_{context}")
        keyboard.button(text="Женщинам без детей 👩", callback_data=f"target_female_nochildren_{context}")
        keyboard.button(text="Мужчинам с детьми 👨👶", callback_data=f"target_male_children_{context}")
        keyboard.button(text="Мужчинам без детей 👨", callback_data=f"target_male_nochildren_{context}")
    ...
    async def process_broadcast_target(self, callback_query: types.CallbackQuery, state: FSMContext):
        ...
        if "target_all" in data:
            ...
            if "children" in data:
                has_children = True
            elif "nochildren" in data:
                has_children = False

            users = self.db.get_targeted_users(gender=gender, has_children=has_children)
        ...
        if gender or has_children is not None:
            report += "\nЦелевая аудитория:"
            if gender == 'female':
                report += "\n- Пол: Женский"
            elif gender == 'male':
